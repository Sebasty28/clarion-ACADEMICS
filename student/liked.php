...
// Get liked posts (including hidden)
$stmt = db()->prepare("
  SELECT p.id, p.title, p.description, p.thumbnail_url, p.like_count, p.created_at, p.updated_at, p.is_published,
...
// Count deleted posts in liked
$deletedStmt = db()->prepare("
  SELECT COUNT(*) as c 
  FROM likes l 
  LEFT JOIN posts p ON p.id = l.post_id 
  WHERE l.student_id = ? AND p.id IS NULL
");
$deletedStmt->execute([$user['id']]);
$deletedCount = (int)($deletedStmt->fetch()['c'] ?? 0);

// Get notifications for liked page
$notificationsStmt = db()->prepare("
  ...
  LIMIT 10
");
$notificationsStmt->execute([$user['id'], $user['id']]);
...
<head>
  ...
  <title>Liked Posts - <?= e(APP_NAME) ?></title>
          ...
          <ul class="nav flex-column">
            <li class="nav-item">
              <a class="nav-link" href="<?= e(BASE_URL) ?>/student/bookmarks.php">
                ...
                Saved Items
                <span class="badge bg-primary rounded-pill ms-auto"><?= $bookmarkCount ?></span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link active" href="<?= e(BASE_URL) ?>/student/liked.php">
                <i class="bi bi-heart-fill me-2"></i>
                Liked Posts
                <span class="badge bg-danger rounded-pill ms-auto"><?= count($likedPosts) ?></span>
              </a>
            </li>
          </ul>
      ...
      <main class="col-12 col-md-9 ms-sm-auto col-lg-10 px-3 px-md-4">
        <div class="d-flex justify-content-between align-items-center mb-4 pt-3">
          <div class="d-flex align-items-center gap-3">
            <h1 style="font-family:'Inter',sans-serif;font-size:30px;font-weight:700;color:#0f172a;margin:0;">Liked Posts</h1>
        ...
        <?php if (!$likedPosts): ?>
          <div class="text-center py-5">
            <i class="bi bi-heart" style="font-size:80px;color:#94a3b8;"></i>
            <p class="mt-3" style="font-family:'Inter',sans-serif;color:#94a3b8;">No liked posts yet</p>
          </div>
        <?php endif; ?>
        ...
        <div class="row g-4" id="postsContainer">
          ...
          ?>
            <div class="col-md-6 col-lg-4" data-hidden="<?= $isHidden ? '1' : '0' ?>">
              <div class="material-card <?= $isHidden ? 'hidden-post' : '' ?>">
                <?php if ($isHidden): ?>
  ...
  <script>
    ...
    subjectFilter.addEventListener('change', function() {
      ...
      document.querySelectorAll('.material-card').forEach(card => {
        ...
        const shouldHide = filterActive && isHidden;
    ...
    // Override toggleLike to remove card when unliked
    ...
    window.toggleLike = async function(postId) {
      ...
      // If unliked, remove the card from view
      const nowLiked = btn?.classList.contains('liked');
      if (wasLiked && !nowLiked && card) {
        card.style.display = 'none';
        
        // Update liked count badge
        if (likedCountEl) {
          const newCount = Math.max(0, parseInt(likedCountEl.textContent || 0) - 1);
          likedCountEl.textContent = newCount;
        }
        
        // Check if no more cards visible
